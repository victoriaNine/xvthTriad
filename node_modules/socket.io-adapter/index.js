...
Adapter.prototype.add = function(id, room, fn){
  ...
  this.rooms[room] = this.rooms[room] || Room({ name: room });
...
Adapter.prototype.createRoom = function(room, options) {
  options      = options || {};
  options.name = room;

  if (!this.rooms[room]) {
    this.rooms[room] = Room(options);
  }
  if (options.callback) process.nextTick(options.callback.bind(null, null));
};
...
Adapter.prototype.del = function(id, room, fn){
  ...
  if (this.rooms.hasOwnProperty(room)) {
    ...
    if (this.rooms[room].length === 0 && !this.rooms[room].permanent) delete this.rooms[room];
  }
