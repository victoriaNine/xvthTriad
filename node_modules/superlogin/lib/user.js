...
module.exports = function (config, userDB, couchAuthDB, mailer, emitter) {
  ...
  this.validateUsername = function (username) {
    ...
    if (!username.match(USER_REGEXP)) {
      return BPromise.resolve('must contain between 3 and 16 lowercase characters');
    }
    if (username.match("_deletedUser")) {
      return BPromise.resolve('is invalid');
    }
  ...
  this.matches = function(value, option, key, attributes) {
    if (attributes && attributes[option] !== value) {
      return option === "confirmPassword" ? "doesn't match the confirmation" : "does not match " + option;
    }
  };
  ...
  var userModel = {
    ...
    whitelist: [
      ...
      'profile.notifVolume',
      'profile.notifyMode',
      'profile.inactiveAudio',
      'profile.knownCards',
      'profile.country',
      'profile.rankPoints',
      'profile.lastRankedGame',
      'profile.daysSinceRG'
    ],
    customValidators: {
    ...
    validate: {
      ...
      username: {
        ...
        length: {
          maximum: 20,
          message: "must be 20 characters long at most"
        }
      },
  ...
  this.create = function (form, req) {
    ...
    return user.process()
      .then(function (result) {
        ...
        return BPromise.reject({error: 'Validation failed', validationErrors: err, status: 200});
      })
      .then(function (hash) {
        // Store password hash
        newUser.local = {};
  ...
  this.createSession = function(user_id, provider, req) {
    ...
    return userDB.get(user_id)
      ...
      .then(function() {
        ...
        newSession.lastLogin = user.lastLogin;
  ...
  this.refreshSession = function (key, renewLease) {
    var newSession;
    return session.fetchToken(key)
      .then(function(oldToken) {
        newSession = oldToken;
        newSession.expires = (renewLease ? newSession.expires : Date.now()) + sessionLife * 1000;
        return BPromise.all([
  ...
  this.changePasswordSecure = function(user_id, form, req) {
    ...
    return changePasswordForm.validate()
      ...
      .then(function(userDoc) {
        ...
        if(user.local && user.local.salt && user.local.derived_key) {
          ...
          if(!form.currentPassword){
            return BPromise.reject({error: 'Password change failed', message: 'You must supply your current password in order to change it.', status: 200});
          }
      ...
      .then(function() {
        ...
        return BPromise.reject(err || {error: 'Password change failed', message: 'The current password you supplied is incorrect.', status: 200});
      })
      .then(function() {
  ...
  this.forgotPassword = function(email, login, req) {
    req = req || {};
    var user, hash;
    return userDB.query('auth/emailUsername', {keys: [email, login], include_docs: true})
      .then(function(result) {
      ...
      .then(function() {
        ...
        return BPromise.resolve({ forgotPassword: user.forgotPassword, email: user.email });
      });
  };
  ...
  this.verifyEmail = function(token, req) {
    ...
    return userDB.query('auth/verifyEmail', {key: token, include_docs: true})
      .then(function(result) {
        if(!result.rows.length) {
          return BPromise.reject({error: 'Invalid verification token', status: 200});
        }
        user = result.rows[0].doc;
  ...
  this.updateProfile = function(user_id, form) {
    var name    = form.name;
    var profile = _.pick(form, _.filter(userModel.whitelist, (key) => { return _.startsWith(key, "profile."); })).profile;

    return userDB.get(user_id)
      .then(function(userDoc) {
        if (name)    { userDoc.name    = name; }
        if (profile) { userDoc.profile = profile; }
        return userDB.put(userDoc);
      });
  };
