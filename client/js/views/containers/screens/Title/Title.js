...
export default Screen.extend({
  ...
  events   : {
    ...
    "click .title_account-logoutBtn" : function () {
      ...
      _$.comm.sessionManager.once("logout", (event, message) => { // eslint-disable-line no-unused-vars
...
function initialize (options = {}) {
  ...
  this.introTL            = null;
  this.accountTemplate    = template(Templ_TitleAccount);
        ...
        _$.comm.sessionManager.validateSession().then(() => {
          ...
          _$.events.once("userDataLoaded", () => {
            ...
            _$.comm.sessionManager.logoutOthers().then(_$.comm.sessionManager.refresh.bind( _$.comm.sessionManager)).then(() => {
              ...
              _$.comm.sessionManager.once("logout", (...args) => { // eslint-disable-line max-nested-callbacks
...
function playIntro () {
  ...
  this.introTL.to(map(logoPaths, "path"), 2, { attr: { fill: "rgba(255, 255, 255, 1)" } }, 4);
...
function openOverlay (overlayName) {
  _$.events.trigger("stopUserEvents");
  const overlaySelector = ".title_overlay-" + overlayName;
...
function doSignup (e) {
  ...
  const data = {
    ...
    profile         : omit(_$.utils.getUserData(), ["version", "userId", "name"])
  };
  ...
  _$.comm.sessionManager.register(data).then((response) => { // eslint-disable-line no-unused-vars
    ...
    let errorString = map(omit(error.validationErrors, "confirmPassword"), error => error.join(". ")).join("<br>");
...
function doLogin () {
  ...
  }).then((session) => { // eslint-disable-line no-unused-vars
...
function toForgotPassword () {
  ...
  const direction = (this.loginConfirmAction === this.doLogin) ? "sendMail" : "login";
  let fromField;
  let toField;
  let messageTxt;
  let forgotPwdTxt;
...
function sendPasswordMail () {
  ...
  const userId = this.$(".title_overlay-login-form .field-login input").val();
  const email  = this.$(".title_overlay-login-form .field-email input").val();
...
function doPasswordReset () {
  ...
  _$.comm.sessionManager.resetPassword({
    ...
    const errors = error.validationErrors.token ? { ...error.validationErrors, token: ["Invalid token"] } : error.validationErrors;
    const errorString = map(errors, error => error.join(". ")).join("<br>");
    ...
    if (has(error.validationErrors, "confirmPassword")) {
...
function updateAccountLayout () {
  const accountLayout = $(this.accountTemplate({
