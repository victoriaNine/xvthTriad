	...
	*/
	...
	Seriously.source('depth', function (source, options, force) {
		...
		*/
		...
		function loadDepthImage (imageData) {
			if (!depthImage) {
		...
		function parseArrayBuffer(arrayBuffer) {
			...
			if (byteArray[0] == 0xff && byteArray[1] == 0xd8) {
				...
				loadDepthImage('data:image/png;base64,' + match[1]);	
			} else {
		...
		if (force) {
			if (source instanceof window.HTMLImageElement && options && options.depthImage) {
				depthImage = source;
				initialize();
			} else if (typeof source === 'string' && source.indexOf("data:image/png;base64,") === 0) {
				loadDepthImage(source);
			} else {
