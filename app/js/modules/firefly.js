define(["underscore"], function Firefly (_) {
    return class Firefly {
        ...
        init (timer) {
            this.size      = parseFloat(_.random(this.options.minSize, this.options.maxSize, true).toFixed(2));
            this.speed     = parseFloat(_.random(this.options.minSpeed, this.options.maxSpeed, true).toFixed(2));
            this.opacity   = parseFloat(_.random(this.options.minOpacity, this.options.maxOpacity, true).toFixed(2));
            ...
            this.varianceX = parseFloat(_.random(0, this.options.maxVarianceX, true).toFixed(2)) * (this.speed / this.options.maxSpeed);
            this.y         = this.drawAreaHeight + parseFloat(_.random(this.size, this.size, true).toFixed(2));

            this.lifeTime  = _.random(this.options.minLifeTime, this.options.maxLifeTime);
