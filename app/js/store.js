...
var events    = clone(Backbone.Events);
...
function timecodeToSecs (timeCode) {
    var timeValues = map(timeCode.split(":"), parseFloat);
...
function getUserData () {
    var data     = omit(_$.state.user.attributes, "album");
...
function getRandomCards (options) {
    ...
    while (i < options.amount) {
        if (isNil(options.level) && isNil(options.minLevel) && isNil(options.maxLevel)) {
            ...
            matches = options.album ? options.album.where({level: level}) : filter(cardList, {level: level});
        }
...
function saveToDb (form = {}) {
    ...
    var profile = omit(data, ["version", "userId", "name", "email"]);
    form = { ...form, name: data.name, profile: profile };
...
function loadFromDb () {
    return _$.comm.sessionManager.getUser.call(_$.comm.sessionManager).then((userData) => {
        ...
        each(omit(userData.profile, "album"), function (value, key) {
...
function loadFromStorage (storageData) {
    ...
    return _decodeSaveData(storageData, "save").then((JSONdata) => {
        _$.app.checkUpdates(JSONdata, (updatedData) => {
            each(omit(updatedData, ["album", "version", "userId"]), function (value, key) {
                _$.state.user.set(key, value);
            });
