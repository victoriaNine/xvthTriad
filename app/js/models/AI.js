...
function getState (currentState, fromCurrentGame) {
    ...
    state.playedCards = map(source.playedCards, function (card) {
        return card.clone();
    });
...
function getPossibleActions (state, depth) {
    var possibleActions    = [];
    var availableCards     = reject(state.playing.get("deck"), "attributes.position");
    var availableCases     = omitBy(state.board);
    var availablePositions = map(availableCases, function (boardCase, caseName) {
